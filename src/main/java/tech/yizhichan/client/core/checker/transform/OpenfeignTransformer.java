        ...
        for (int i = 0; i < lines.length; i++) {
            ...
            if (index != null) {
                ...
                line = StringUtils.replace(line, feignClientPropertyName + StrUtil.DOT + method + "(",
                        String.format("cn.hutool.extra.spring.SpringUtil.getBean(tech.yizhichan.client.generic.openfeign.DynamicFeignClient.class).%s(\"%s\", \"%s\", ", getDynamicFeignClientMethodByHttpMethod(httpMethod), feignClientName, url));
